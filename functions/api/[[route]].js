...
const ADMIN_TOKEN = '8721'; // змініть на свій пароль
...
// ── Telegram сповіщення ───────────────────────────────────────
const TG_BOT_TOKEN = '8712898544:AAGLysJP0M_TAnllYe4Ib_qITapDUYFfC6Q';
const TG_CHAT_IDS  = ['1041579057', '7443699603']; // обидва одержувачі

async function sendTelegramNotification(env, orderData) {
  const botToken = env.TELEGRAM_BOT_TOKEN || TG_BOT_TOKEN;
  ...
  const deliveryMap = {
    ...
    pickup:      '🏪 Самовивіз (Кропивницький)',
    courier:     '🛵 Кур\'єр (Кропивницький)',
  };

  const deliveryLabel = deliveryMap[orderData.delivery] || orderData.delivery || '—';
  const cityLine    = orderData.city    ? `\n📍 Місто/відділення: ${orderData.city}`    : '';
  const commentLine = orderData.comment ? `\n💬 Коментар: ${orderData.comment}` : '';

  const text = [
    `🛒 *НОВЕ ЗАМОВЛЕННЯ — RK Perfume*`,
    ``,
    `👤 Імʼя: ${orderData.name}`,
    `📞 Телефон: ${orderData.phone}`,
    `🧴 Аромат: ${orderData.product}`,
    `📦 Доставка: ${deliveryLabel}${cityLine}${commentLine}`,
    ``,
    `🕐 ${new Date().toLocaleString('uk-UA', { timeZone: 'Europe/Kyiv' })}`,
  ].join('\n');

  // Відправляємо одночасно всім одержувачам
...
async function seedIfEmpty(db) {
  ...
  for (let i = 0; i < DEFAULT_PRODUCTS.length; i++) {
    ...
    await db.prepare(
      'INSERT INTO products (category,brand,name,desc,notes,volume,price,type,featured,sort_order,image_url) VALUES (?,?,?,?,?,?,?,?,?,?,?)'
  ...
  // Add image_url column if not exists (migration for existing DBs)
  try { await db.prepare('ALTER TABLE products ADD COLUMN image_url TEXT DEFAULT NULL').run(); } catch(e) {}
...
export async function onRequest(context) {
  ...
  if (path.startsWith('/products/') && method === 'PUT') {
    ...
    const notes = Array.isArray(body.notes) ? body.notes.join(',') : (body.notes || '');
    await db.prepare(
      'UPDATE products SET category=?,brand=?,name=?,desc=?,notes=?,volume=?,price=?,type=?,featured=?,image_url=? WHERE id=?'
  ...
  if (method === 'POST' && path.match(/^\/products\/\d+\/image$/)) {
    if (!checkAuth(request)) return err('Unauthorized', 401);
    const pid = path.split('/')[2];
    const body = await request.json();
    const imageUrl = body.image_url || null;
    await env.DB.prepare('UPDATE products SET image_url=? WHERE id=?').bind(imageUrl, pid).run();
    return json({ ok: true });
  }
  ...
  if (path === '/order' && method === 'POST') {
    const body = await request.json();
    const { name, phone, product, delivery, city, comment } = body;

    if (!name || !phone || !product) {
      return err('Заповніть обовʼязкові поля', 400);
    }
    ...
    // Зберегти в базу
    await db.prepare(
      'INSERT INTO orders (name, phone, product, delivery, city, comment, created_at) VALUES (?,?,?,?,?,?,?)'
    ).bind(name, phone, product, delivery || '', city || '', comment || '', createdAt).run();

    // Відправити в Telegram обом одержувачам
    await sendTelegramNotification(env, body);

    return json({ ok: true });
  }

  // ── GET /api/orders (адмін) ────────────────────────────────
